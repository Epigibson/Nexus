...
if [ -f "$HOME/.bashrc" ]; then
    ...
    # Con lo podemos eliminar de manera precisa:
...
if [ -f "$HOME/.zshrc" ]; then
    # Create backup before modifying
    BACKUP_FILE="$HOME/.zshrc.bak.$(date +%s)"
...
echo "💡 Si necesitas restaurar tus configuraciones originales, revisa los archivos .bak.* en tu home."
